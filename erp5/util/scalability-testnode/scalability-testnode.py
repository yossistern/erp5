...
class NodeTestSuite(SlapOSInstance):
  ...
  def createSuiteLog(self):
    # /srv/slapgrid/slappartXX/srv/var/log/scalability-estnode/az-mlksjfmlk234Sljssdflkj23KSdfslj/suite.log
      ...
      while True:
        try:
          ...
          continue
            ...
            log("erp5-scalability-testnode exception", exc_info=sys.exc_info())
            raise
        now = time.time()
        self.cleanUp(test_result)
        if (now-begin) < 120:
